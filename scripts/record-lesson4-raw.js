#!/usr/bin/env node
/**
 * record-lesson4-raw.js
 * Opens a VISIBLE browser window running Lesson 4 for OBS to capture.
 * Pauses after launch so you can aim OBS's Window Capture at the window,
 * then plays through automatically until ozmeva_lesson4_complete fires.
 *
 * Run:   node scripts/record-lesson4-raw.js
...
(async () => {
  console.log('\n════════════════════════════════════════════');
  console.log('  LESSON 4 — OBS RECORDER (HEADED MODE)');
  console.log('════════════════════════════════════════════\n');
  ...
  try {
    ...
    // Ensure Lesson 4 start button is visible, then wait for OBS setup
    await page.waitForSelector('#ek-start-lesson4', { timeout: 15000 });
    console.log('\n  ✓ Browser ready — Lesson 4 card visible.');
    ...
    console.log('\n  → Starting Lesson 4...');
    await page.locator('#ek-start-lesson4').click();
    await page.waitForSelector('#ek-lesson-player', { state: 'visible', timeout: 10000 });
    console.log('  ✓ Lesson 4 player open — lesson running\n');
    ...
    while (Date.now() - pollStart < MAX_MS) {
      ...
      const complete = await page.evaluate(
        () => localStorage.getItem('ozmeva_lesson4_complete') === 'true'
      ).catch(() => false);

      if (complete) {
        console.log(`  ✓ ozmeva_lesson4_complete set — lesson finished at ${elapsed}s`);
        break;
      }
  ...
  console.log('    content/lesson4/lesson4-raw.mp4');
  console.log('════════════════════════════════════════════\n');
