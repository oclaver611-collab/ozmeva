#!/usr/bin/env node
/**
 * record-lesson3-raw.js
 * Opens a VISIBLE browser window running Lesson 3 for OBS to capture.
 * Pauses after launch so you can aim OBS's Window Capture at the window,
 * then plays through automatically until ozmeva_lesson3_complete fires.
 *
 * Run:   node scripts/record-lesson3-raw.js
...
(async () => {
  console.log('\n════════════════════════════════════════════');
  console.log('  LESSON 3 — OBS RECORDER (HEADED MODE)');
  console.log('════════════════════════════════════════════\n');
  ...
  try {
    ...
    await context.addInitScript(() => {
      ...
      localStorage.removeItem('ozmeva_lesson3_progress');
    ...
    // Ensure Lesson 3 start button is visible, then wait for OBS setup
    await page.waitForSelector('#ek-start-lesson3', { timeout: 15000 });
    console.log('\n  ✓ Browser ready — Lesson 3 card visible.');
    ...
    console.log('\n  → Starting Lesson 3...');
    await page.locator('#ek-start-lesson3').click();
    await page.waitForSelector('#ek-lesson-player', { state: 'visible', timeout: 10000 });
    console.log('  ✓ Lesson 3 player open — lesson running\n');
    ...
    while (Date.now() - pollStart < MAX_MS) {
      ...
      const complete = await page.evaluate(
        () => localStorage.getItem('ozmeva_lesson3_complete') === 'true'
      ).catch(() => false);

      if (complete) {
        console.log(`  ✓ ozmeva_lesson3_complete set — lesson finished at ${elapsed}s`);
        break;
      }
  ...
  console.log('    content/lesson3/lesson3-raw.mp4');
  console.log('════════════════════════════════════════════\n');
